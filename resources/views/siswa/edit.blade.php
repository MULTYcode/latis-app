...
@section('content')
<div class="container mx-auto px-4 py-6" id="edit-siswa-container" data-success-message="{{ session('success') ?? '' }}" data-error-messages="@if($errors->any()){{ implode('<br>', $errors->all()) }}@endif">
        ...
        <div class="mb-4">
            ...
            <select id="lembaga_id" name="lembaga_id" class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                @foreach ($lembaga as $item)
                    <option value="{{ $item->id }}" {{ old('lembaga_id', $siswa->lembaga_id) == $item->id ? 'selected' : '' }}>
                        {{ $item->nama }}
                    </option>
                @endforeach
            </select>
